...
describe('A2A Integration Tests', () => {
  ...
  describe('Concurrent Request Handling', () => {
    it('should handle multiple concurrent requests', async () => {
        ...
        // Check if we have enough credits for 3 requests (30 credits)
        const requiredCredits = 30n
        if (initialCredits < requiredCredits) {
          console.log(`[TEST] Insufficient credits for 3 concurrent requests. Available: ${initialCredits}, Required: ${requiredCredits}`)
          
          // Try with fewer requests based on available credits
          const maxRequests = Number(initialCredits / 10n)
          if (maxRequests < 1) {
            console.log(`[TEST] Not enough credits for even 1 request. Skipping test.`)
            return
          }
          
          console.log(`[TEST] Trying with ${maxRequests} requests instead of 3`)
          
          // Send fewer concurrent requests based on available credits
          const promises = Array.from({ length: maxRequests }, (_, i) => 
            testContext.sendMessageAndValidate(
              A2ATestUtils.createTestMessage(`Concurrent request ${i + 1}`)
            )
          )
          ...
          // Validate total credits burned
          const expectedBurned = BigInt(maxRequests) * 10n
          await testContext.validateCreditsBurned(initialCredits, expectedBurned)
        } else {
          ...
          // Validate total credits burned
          await testContext.validateCreditsBurned(initialCredits, 30n)
        }
    }, TEST_CONFIG.TIMEOUT)
  })
})
