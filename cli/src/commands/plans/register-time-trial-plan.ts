 ...
 */
export default class RegisterTimeTrialPlan extends BaseCommand {
  ...
  public async run(): Promise<void> {
    ...
    try {
      const result = await payments.plans.registerTimeTrialPlan(await this.parseJsonInput(flags['plan-metadata']), await this.parseJsonInput(flags['price-config']), await this.parseJsonInput(flags['credits-config']))
