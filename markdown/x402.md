...
can only cost you a failed verification: it binds nothing. The SDK warns when
`resource`/`httpVerb` are passed without `tokenVersion: 3`; the CLI refuses the
combination outright, and the OpenClaw tools drop it.
