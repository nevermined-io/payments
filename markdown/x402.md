...
v3 is **opt-in**: pass `tokenVersion: 3` along with the resource binding.
...
which also binds the token to the agent's service endpoint.
...
**MPP does not pin v3.** One MPP access token is presented across many
challenges — genuinely distinct operations, not retries of one — so a per-token
nonce would reject every buyer's second challenge. MPP therefore crosses to v3 on
the same schedule as x402, once clients mint one token per paid request;
`payments.mpp.fetch` already mints a fresh token per credential it presents.
