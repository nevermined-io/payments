...
### Nano (XNO) settlement reference

Beyond the two schemes the SDK resolves (`nvm:erc4337` and `nvm:card-delegation`), the
x402 exact-scheme specification also accepts `nano:mainnet` as a settlement network. Nano
(XNO) is a feeless, sub-second-finality, self-custodial rail: an agent's own key signs one
block (no gas token, no relayer, no freezeable stablecoin), which is the same "make the
value rail additive, not a fork" shape as adding another `resolveScheme()` branch. A
mainnet Nano-x402 settlement is demonstrated in the
[agent-wallet-multirail](https://github.com/PANDeveloper001/agent-wallet-multirail)
reference (a tested `PaymentRail` example that settles the same amount on Nano, USDC,
Skyfire or Payman behind one interface), and the exact-nano network spec is tracked at
[x402-foundation/x402#3432](https://github.com/x402-foundation/x402/pull/3432). This SDK
does not resolve a Nano scheme today; the reference above is the wire shape a Nano scheme
would follow when it is added.
