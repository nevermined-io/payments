...
8. **Never Retry a Spent Token**: Treat `BCK.X402.0059` as "mint a new token", not as a transient settlement failure
9. **Relay Tokens Verbatim**: Do not re-encode, trim or normalise `payment-signature` — a v3 envelope that disagrees with its signature is rejected as forgery
